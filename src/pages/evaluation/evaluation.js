...
export default function Evaluations(props) {
    ...
    useEffect(() => {
        ...
        getAllEvaluation(page, rowLimit).then((res) => {
            ...
            if (res.status) {
                //  const numdescending = [...res.results].sort((a, b) => b.id - a.id);
                 const strDescending = [...res.results].sort((a, b) =>
                 a.createdAt > b.createdAt ? -1 : 1,);
             
                //setting the fetched Course into state variable
                setEvaluation(strDescending);
                setTotalPages(strDescending.totalPages)
              
                console.log('Evaluation Data', strDescending);
            }
            else {
